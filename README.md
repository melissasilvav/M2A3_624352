Reemplacé etiquetas genéricas como `<div>` y `<strong>` por etiquetas semánticas como `<header>`, `<nav>`, `<main>`, `<article>`, `<aside>` y `<footer>`, lo que hace que el código sea más claro y accesible.  
